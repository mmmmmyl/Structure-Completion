...
void Wang_GetCurve(const vector<Point>& mouse_points, vector<Point>& PointList)
{
	...
	for (int i = 0; i < mouse_points.size() - 1; i++)
	{
		...
		if ((mouse_points[i].x - mouse_points[i + 1].x) * (mouse_points[i].x - mouse_points[i + 1].x) + (mouse_points[i].y - mouse_points[i + 1].y) * (mouse_points[i].y - mouse_points[i + 1].y) > 20)continue;
